...
def draw_heading(word, compact=False):
    ...
    compact=True makes the image only as wide as the word plus a short rule.
    That matters inside <summary>: a full-width image wraps onto its own line
    below GitHub's disclosure triangle, but a narrow one sits inline next to it,
    so the triangle and the heading line up.
    """
    FS = 16
    H = 24
    text_end = len(word) * FS * 0.6 + 14
    rule_end = text_end + 46 if compact else WIDTH
    width = int(rule_end + 8) if compact else WIDTH
    p = [head(width, H, font=font_head())]
    p.append(label(0, 17, word, FS, "e-f", extra=' font-weight="600"'))
    p.append(f'<line x1="{text_end:.0f}" y1="11.5" x2="{rule_end:.0f}" '
             f'y2="11.5" class="u-s" stroke-width="1"/>')
    p.append("</svg>")
    return "".join(p)
...
def main():
    ...
    files = {"stats.svg": draw_stats(s), "year.svg": draw_year(s),
             "hd-stats.svg": draw_heading("stats")}
    for word in ("about", "stack", "projects"):   # these live inside <summary>
        files[f"hd-{word}.svg"] = draw_heading(word, compact=True)
